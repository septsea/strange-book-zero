...
\begin{remark}
    整环 $D$ 里不一定有名为 ${} \pm 2$, ${}\pm 3$, $\cdots$ 的元. 回忆一下, 若 $a \in D$, $n \in \mathbb{N}$, 则
    \begin{align*}
        na = n \cdot a = \underbrace{a + a + \cdots + a}_{\text{$n$ $a$\apostrophe s}}.
    \end{align*}
    若 $-n \in \mathbb{N}$, 则
    \begin{align*}
        na = -((-n)a).
...
\begin{example}
    取 $f(x) = x^6 - x^3 + 1 \in D[x]$. 若 $D = \mathbb{F}$, 则
    \begin{align*}
...
\begin{pf}
    我们证明 (\ding{72}), 将剩下的二个式留给读者作练习. 首先, 任取 $i$, $j \in \mathbb{N}$, $p$, $q \in D$, 有
    \begin{align*}
