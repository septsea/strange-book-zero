...
\begin{definition}
    ...
    \begin{align*}
        D[x] = \{\, a_0 x^0 + a_1 x^1 + \cdots + a_n x^n \mid n \in \mathbb{N},\ a_0,a_1,\cdots,a_n \in D \,\}.
    \end{align*}
...
\begin{example}
    $0y^0 + 1y^1 + (-1)y^2 + 0y^3 + (-7)y^4 \in \mathbb{Z}[y]$ 是一个 $4$ 次整式. 顺便一提, 一般把 $y^1$ 写为 $y$. 这个整式的一个更普通的写法是
    \begin{align*}
        y - y^2 - 7y^4.
    \end{align*}
...
\begin{example}
    取 $\mathbb{Z}[x]$ 的二个元 $f(x)=x^0 + 2x^2$, $g(x)=-3x^0 + 4x - x^3$. 先改写一下:
    \begin{align*}
...
\begin{example}
    $y - y^2 - 7y^4 \in \mathbb{Z}[x]$ 可以写为 $-7y^4 - y^2 + y$, 其首项是 $-7y^4$, 且其首项系数是 $-7$.
\end{example}
...
\begin{remark}
    ...
    这{\scriptsize 儿}还有一件事{\scriptsize 儿}值得一提. 考虑
    \begin{align*}
