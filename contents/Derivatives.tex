...
也许这不太能体现 (\myStar) 的作用: 算二个多项式积的导数时, 先拆再算好像没什么不方便的\period 的确如此\period 可是 (\myStar) 的推论
\begin{align*}
...
\begin{proposition}
    设 $f(x)$, $g(x) \in D[x]$\period 则 $f(x)$ 与 $g(x)$ 的复合的导数适合链规则 \term{the chain rule}:
    \begin{align*}
        (g \circ f)^{\prime} (x) = (g^{\prime} \circ f)(x) f^{\prime} (x) \period
    \end{align*}
\end{proposition}
