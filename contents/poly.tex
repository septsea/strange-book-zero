...
\begin{example}
    这里举一个 ``平凡的'' \term{trivial} 例子\period $N$ 只有一个元 $0$\period 可以验证, $N$ 关于普通加法与乘法作成群\period 这也是 ``最小的环''\period 在上个例子里, 取 $k=0$ 就是 $N$\period
\end{example}

\begin{example}
    这里举一个 ``不平凡的'' \term{nontrivial} 例子\period 设 $R = \{\, 0,a,b,c \,\}$\period 加法和乘法由以下两个表给定:
    \begin{align*}
        \arraycolsep=0.25cm
        \begin{array}{c|cccc}
            + & 0 & a & b & c \\ \hline
...
\begin{example}
    % /* cspell: disable-next-line */
    看一个有限整环的例子\period 设 $V$ \term{Vierergruppe}\myFN{A German word which means \textit{four-group}.} 是 $4$ 元集:
    \begin{align*}
...
\begin{example}
    现在看一个不那么平凡的例子\period 设
    \begin{align*}
        D = \{\, x + y\sqrt{3} \mid x,y \in \ZZ \,\} \period
    \end{align*}
    这个 $D$ (关于数的运算) 作成整环\period

    首先, 我们说, 不存在有理数 $q$ 使 $q^2 = 3$\period 用反证法\period 设 $q = \frac{m}{n}$, $m$, $n$ 是非零整数\period 我们知道, 分数可以约分, 故可以假设 $m$, $n$ 都不是 $3$ 的倍数\period 这样
    \begin{align*}
        m^2 = 3n^2 \period
    \end{align*}
    所以 $m^2$ 一定是 $3$ 的倍数\period 因为
    \begin{align*}
         & (3\ell)^2 = 3 \cdot 3\ell^2,                \\
         & (3\ell \pm 1)^2 = 3(3\ell^2 \pm 2\ell) + 1,
    \end{align*}
    故由此可看出, $m$ 也是 $3$ 的倍数\period 记 $m=3u$\period 这样
    \begin{align*}
        3u^2 = n^2 \period
    \end{align*}
    所以 $n$ 也是 $3$ 的倍数\period 这跟假设矛盾!
...
\begin{definition}
    ...
    \end{align*}
    的表达式称为 $D$ 上 $x$ 的一个多项式 \term{polynomial in $x$ over $D$}\period $n$ 称为其次数 \term{degree}, $a_i$ 称为其 $i$ 次系数 \term{the $i^\text{th}$ coefficient}, $a_i x^i$ 称为其 $i$ 次系数 \term{the $i^\text{th}$ coefficient}\period $f(x)$ 的次数可写为 $\deg f(x)$\period

    若二个多项式的次数与各同次系数均相等, 则二者相等\period

    多项式的系数为 $0$ 的项可以不写\period

    约定 $0 \in D$ 也是多项式, 称为零多项式\period 零多项式的次数是 $-\infty$\period 任取整数 $m$, 约定
    \begin{align*}
...
\begin{example}
    $0x^0 + 1x^1 + (-1)x^2 + 0x^3 + (-7)x^4 \in \ZZ[x]$ 是一个 $4$ 次多项式\period 顺便一提, 一般把 $x^1$ 写为 $x$\period 这个多项式的一个更普通的写法是
    \begin{align*}
        x - x^2 - 7x^4 \period
    \end{align*}
    也就是说, 按照中学的习惯写多项式即可\period
\end{example}
