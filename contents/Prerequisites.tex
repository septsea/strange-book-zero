...
\begin{definition}
    设 $a$, $b$ 是实数\period $a + b\ii$ 的共轭 \term{conjugate} 是复数 $a - b\ii$ \period 复数 $z_1$ 的共轭可写为 $\overline{z_1}$\period
\end{definition}
...
\begin{example}
    全体偶数作成的集也是环\period 一般地, 设 $k$ 是整数, 则全体 $nk$ ($n \in \ZZ$) 作成的集是环\period
\end{example}
...
\begin{example}
    $\FF$ 也是整环\period 它有无限多个单位: 任意 $\FF^{\ast}$ 的元都是单位\period
\end{example}
...
\begin{example}
    ...
    首先, 我们说, 不存在有理数 $q$ 使 $q^2 = 3$\period 用反证法\period 设 $q = \frac{m}{n}$, $m$, $n$ 是非零整数\period 我们知道, 分数可以约分, 故可以假设 $3$ 不是 $m$ 与 $n$ 的公因子\period 这样
    \begin{align*}
        m^2 = 3n^2 \period
    \end{align*}
    所以 $3$ 一定是 $m^2$ 的因子\period 因为
    \begin{align*}
         & (3\ell)^2 = 3 \cdot 3\ell^2,                \\
         & (3\ell \pm 1)^2 = 3(3\ell^2 \pm 2\ell) + 1,
    \end{align*}
    故由此可看出, $3$ 也是 $m$ 的因子\period 记 $m=3u$\period 这样
    \begin{align*}
        3u^2 = n^2 \period
    \end{align*}
    所以 $3$ 也是 $n$ 的因子\period 这跟假设矛盾!
