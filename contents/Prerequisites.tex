...
作者假定读者会初中算学. 具体地说, 作者假定读者至少会如下知识\myFN{感谢知乎用户 ``咸鱼晓孔'' 先生的文章: \texttt{https://zhuanlan.zhihu.com/p/305969765}.}:
\begin{quotation}
...
命题就介绍到这里. 下面回顾复数基础.\myFN{如果读者没学过复数, 也不用怕. 本文不要求读者很了解复数; 本文只要求读者看到 ``$\mathrm{i}$'' 不觉得陌生、恐惧.}

\begin{definition}
    复数 \term{complex number} 是形如 $x + y \mathrm{i}$ ($x$, $y$ 是实数) 的数.
\end{definition}

\begin{remark}
    可将 $x + y \mathrm{i}$ 写为 $x + \mathrm{i} y$.
\end{remark}

\begin{definition}
    设 $a,b,c,d$ 是实数. 则
    \begin{align*}
        a + b \mathrm{i} = c + d \mathrm{i} \iff a = c \text{ and } b = d.
    \end{align*}
\end{definition}

\begin{remark}
    我们把形如 $a+0 \mathrm{i}$ 的复数写为 $a$, 并认为 $a+0 \mathrm{i}$ 是实数. 反过来, $a$ 也可以认为是复数 $a + 0 \mathrm{i}$.

    形如 $0 + b \mathrm{i}$ 的复数可写为 $b \mathrm{i}$. 按照习惯, $1\mathrm{i}$ 可写为 $\mathrm{i}$, 且 $-1\mathrm{i}$ 可写为 $-\mathrm{i}$.
...
\begin{example}
    我们计算 $\mathrm{i}$ 与自己的积:
    \begin{align*}
        \mathrm{i} \cdot \mathrm{i} = (0 + 1\mathrm{i}) (0 + 1\mathrm{i}) = (0 \cdot 0 - 1 \cdot 1) + (0 \cdot 1 + 1 \cdot 0) \mathrm{i} = -1.
    \end{align*}
    简单地说, 就是
    \begin{align*}
        \mathrm{i} \cdot \mathrm{i} = \mathrm{i}^2 = -1.
    \end{align*}
\end{example}

设 $z_1, z_2, z_3$ 是任意三个复数 (不必不同). 设 $z_1 = a + b \mathrm{i}$.
...
\begin{proposition}
    ...
    (iv) 存在复数 $w = (-a) + (-b) \mathrm{i}$ 使 $w + z_1 = 0$.

    通常把适合 (iv) 的 $w$ 记为 $-z_1$, 且称之为 $z_1$ 的相反数.
\end{proposition}

\begin{remark}
    $(-a) + (-b) \mathrm{i}$ 可写为 $-a-b \mathrm{i}$.
\end{remark}
...
\begin{proposition}
    ...
    (ix) 若 $z_1 \neq 0$, 则存在复数 $v = \frac{a}{a^2 + b^2} + \frac{-b}{a^2 + b^2} \mathrm{i}$ 使 $v z_1 = 1$.
...
\begin{remark}
    $a$, $b\mathrm{i}$, $c$, $d\mathrm{i}$ 都可以看成是复数. 这样
    \begin{align*}
        (a + b \mathrm{i}) (c + d \mathrm{i})
        = {} & (a + b \mathrm{i}) c + (a + b \mathrm{i}) (d \mathrm{i})         \\
        = {} & ac + b \mathrm{i} c + a d \mathrm{i} + b \mathrm{i} d \mathrm{i} \\
        = {} & ac + bc\mathrm{i} + ad\mathrm{i} + bd\mathrm{i}^2                \\
        = {} & (ac + bd\mathrm{i}^2) + (ad + bc)\mathrm{i}                      \\
        = {} & (ac - bd) + (ad + bc)\mathrm{i}.
    \end{align*}
    也就是说, 我们不必死记复数的乘法规则: 只要用运算律与 $\mathrm{i}^2 = -1$ 即可召唤它.
\end{remark}

\begin{definition}
    设 $a$, $b$ 是实数. $a + b\mathrm{i}$ 的共轭 \term{conjugate} 是复数 $a - b\mathrm{i}$. 复数 $z_1$ 的共轭可写为 $\overline{z_1}$.
\end{definition}

\begin{proposition}
    共轭适合如下性质:

    (x) $\overline{z_1} + z_1$ 与 $\mathrm{i} \cdot (\overline{z_1} - z_1)$ 都是实数;
...
复数就先回顾到这里. 下面回顾算学归纳法.\myFN{如果读者之前从未接触算学归纳法, 那么读者可从本文开始慢慢地熟悉它.}

\begin{remark}
    算学归纳法 \term{mathematical induction} 是一种演绎推理.
\end{remark}
...
\begin{remark}
    可以这么理解算学归纳法. 假设有一排竖立的砖. 如果 (i) 第一块砖倒下, 且 (ii) 前一块砖倒下可引起后一块砖倒下, 那么所有的砖都可以倒下, 是吧? 由此也可以看出, (i) (ii) 缺一不可. 第一块砖不倒, 后面的砖怎么倒下呢?\myFN{当然, 也可以从第 $n$ 块砖开始倒下 ($n > 1$), 但这就照顾不到第一块了.} 如果前一块砖倒下时后一块砖不一定能倒下, 那么会在某块砖后开始倒不下去.
\end{remark}
...
\begin{example}
    ...
    \end{align*}
    故我们由 $P(\ell)$ 推出了 $P(\ell + 1)$. 我们在哪{\scriptsize 儿}用到了 $P(\ell)$ 呢? 我们在标了 (IH) 的那一行用了 $P(\ell)$. 这样的假设称为归纳假设 \term{induction hypothesis}.
...
\begin{example}
    ...
    不同的老婆\myFN{一般地, 2 次元人会称动画、漫画、游戏、小说中自己喜爱的女性角色为老婆 \term{waifu}. 一个 2 次元人可以有不止一个老婆. 不过, 有些时候, 就算某角色 $c$ 是男性角色, $c$ 也可以是某个 2 次元人的老婆. 关于这一点, 读者可参考 Namie 先生 (微博: \texttt{@bananaNamie}; 鸟叫: \texttt{@namgoreng}) 画的一个角色: 水月 \term{Mizuki}.}有着不同的发色. 但是, 我们用算学归纳法却可以 ``证明'', 任意的 $n$ ($n \geq 1$) 个老婆有着相同的发色! 称这个命题为 $Q(n)$. 这里, $n_0$ 自然取 $1$.
...
\begin{example}
    全体整数作成的集用 $\mathbb{Z}$ \term{Zahl}\myFN{A German word which means \textit{number}.} 表示. 它可以写为
    \begin{align*}
        \mathbb{Z} = \{\, 0,1,-1,2,-2,\cdots,n,-n,\cdots \,\}.
    \end{align*}
\end{example}

\begin{example}
    全体非负整数作成的集用 $\mathbb{N}$ \term{natural} 表示. 它可以写为
    \begin{align*}
...
\begin{example}
    全体有理数作成的集用 $\mathbb{Q}$ \term{quotient} 表示. 因为整数是有理数, 所以 $\mathbb{Z} \subset \mathbb{Q}$. 因为有理数 $\frac12$ 不是整数, 我们说 $\mathbb{Z}$ 是 $\mathbb{Q}$ 的真子集.
\end{example}

\begin{definition}
    全体实数作成的集用 $\mathbb{R}$ \term{real} 表示.
\end{definition}

\begin{definition}
    全体复数作成的集用 $\mathbb{C}$ \term{complex} 表示. 不难看出,
    \begin{align*}
...
\begin{definition}
    $\mathbb{F}$ \term{field} 可表示 $\mathbb{Q}$, $\mathbb{R}$, $\mathbb{C}$ 的任意一个. 不难看出, $\mathbb{F}$ 适合这几条:
...
\begin{definition}
    设 $\mathbb{L}$ 是 $\mathbb{C}$, $\mathbb{R}$, $\mathbb{Q}$, $\mathbb{Z}$, $\mathbb{N}$, $\mathbb{F}$ 的任意一个. $\mathbb{L}^{\ast}$ 表示 $\mathbb{L}$ 去掉 $0$ 后得到的集. 不难看出, $\mathbb{L}$ 是 $\mathbb{L}^{\ast}$ 的真超集.
\end{definition}
...
\begin{definition}
    ...
    称 $A$ 是定义域 \term{domain}, $B$ 是陪域\myFN{不要混淆陪域与象集 (\textit{image}, \textit{range}). $f$ 的象集是
        \begin{align*}
            \operatorname{Im} f = \{\, b \in B \mid b = f(a), \, a \in A \,\}.
        \end{align*}
        这就是中学算学里的 ``值域''.
    } \term{codomain}.
\end{definition}
...
\begin{example}
    可以把 $\mathbb{R} \times \mathbb{R}$ 看作平面上的点集.
    \begin{align*}
         & \mathbb{R} \times \mathbb{R} \to \mathbb{R},  \tag*{$f$:} \\
...
\begin{example}
    设 $A \subset \mathbb{C}$, 且 $A$ 非空. 定义二个 $A$ 到 $\mathbb{C}$ 的函数: $f_1 (x) = x^2$, $f_2 (x) = |x|^2$. 如果 $A = \mathbb{R}$, 那么 $f_1 = f_2$. 可是, 若 $A = \mathbb{C}$, $f_1$ 与 $f_2$ 不相等.
\end{example}

\begin{example}
    设 $A$ 是全体正实数作成的集. 定义二个 $A$ 到 $\mathbb{R}$ 的函数: $f_1 (x) = \frac16 \log_{2} {x^3}$, $f_2 (x) = \log_{4} {x}$. 知道对数的读者可以看出, $f_1$ 与 $f_2$ 有着相同的对应法则, 故 $f_1 = f_2$. 因为 $f_2$ 是对数函数 \term{logarithmic function}, 所以 $f_1$ 也是.\myFN{如果读者没学过对数, 可放心地跳过本例哟!}
\end{example}

\begin{remark}
    在语境清楚的情况下, 可以单说函数的对应法则. 比如, 中学算学课说 ``$2$ 次函数 $f(x) = x^2 + x - 1$'' 时, 定义域与陪域默认都是 $\mathbb{R}$. 中学的函数一般都是实数的子集到实数的子集的函数. 所谓 ``自然定义域'' 是指 (在一定范围内) 一切使对应法则有意义的元构成的集. 比如, 在中学, 我们说 $\frac{1}{x}$ 的自然定义域是 $\mathbb{R}^{\ast}$, $\sqrt{x}$ 的自然定义域是一切非负实数. 在研究复变函数时, 我们说 $\frac{1}{z}$ 的自然定义域是 $\mathbb{C}^{\ast}$. 如果不明确函数的定义域, 我们会根据语境作出自然定义域作为它的定义域.
\end{remark}
...
\begin{definition}
    $A \times A$ 到 $A$ 的函数称为 $A$ 的二元运算 \term{binary function}.
\end{definition}

\begin{example}
    设 $f(x,y) = x-y$. 这个 $f$ 是 $\mathbb{Z}$ 的二元运算; 但是, 它不是 $\mathbb{N}$ 的二元运算.
\end{example}
...
\begin{example}
    设 $\mathbb{F}_{\mathrm{nu}}$ 是将 $\mathbb{F}$ 去掉 $0$, $1$ 后得到的集\myFN{这个 $\mathbb{F}_{\mathrm{nu}}$ 只是临时记号: $\mathrm{nu}$ 表示 \textit{nil}, \textit{unity}.}. 看下列 $6$ 个法则:
    \begin{align*}
    ...
    \end{align*}
    记 $S_6 = \{\, f_0,f_1,f_2,f_3,f_4,f_5 \,\}$. 可以验证, $S_6 \subset T(\mathbb{F}_{\mathrm{nu}})$.
...
\begin{example}
    $\mathbb{Z}$ 的加法当然适合结合律. 可是, 它的减法不适合结合律.
\end{example}
...
\begin{example}
    $\mathbb{F}^{\ast}$ 的乘法当然适合交换律. 可是, 它的除法不适合交换律.
\end{example}
...
\begin{remark}
    在本文里, $\cdot$ 运算的优先级 \term{priority} 高于 $+$ 运算. 所以, $a \cdot b + c$ 的意思就是
    \begin{align*}
        (a \cdot b) + c,
    \end{align*}
    而不是
...
\begin{definition}
    ...
    \end{align*}
    则说 $+$ 与 $\cdot$ 适合左 ($\cdot$) 分配律\myFN{在不引起歧义时, 括号里的内容可省略. 或者这么说: 当我们说 $+$, $\cdot$ 适合分配律时, 我们不会理解为 $x + (y \cdot z) = (x + y) \cdot (x + z)$. 但有意思的事{\tiny 儿}是, 如果把 $+$ 理解为并, $\cdot$ 理解为交, $x$, $y$, $z$ 理解为集, 那这个式是对的. 当然, $x \cdot (y + z) = x \cdot y + x \cdot z$ 也是对的.} \term{left distributivity}. 类似地, 若
    \begin{align*}
...
\begin{example}
    $\mathbb{F}$ 的加法与乘法适合分配律. 当然, 减法与乘法也适合分配律:
    \begin{align*}
...
\begin{example}
    显然, $\mathbb{N}$ 的乘法不适合消去律, 但 $\mathbb{N}^{\ast}$ 的乘法适合消去律\myFN{后面提到整环时, 我们会稍微修改一下消去律的描述.}.
\end{example}

\begin{example}
    考虑 $x \circ y = x^3 + y^2$. 若把 $\circ$ 视为 $\mathbb{N}$ 的二元运算, 那么它适合消去律. 若把 $\circ$ 视为 $\mathbb{Q}$ 的二元运算, 那么它适合右消去律. 若把 $\circ$ 视为 $\mathbb{C}$ 的二元运算, 那么它不适合任意一个消去律\myFN{考虑复数 $-\frac{1}{2} + \mathrm{i} \frac{\sqrt{3}}{2}$; 它的立方是 $1$. 现在, 或许这个例有点难; 不过, 当读者学到后面时, 或许读者会对此有更深的理解吧!}.
\end{example}
...
\begin{definition}
    ...
    \begin{align*}
        e \circ x = x \circ e = x,
    \end{align*}
    则说 $e$ 是 $A$ 的 (关于运算 $\circ$ 的) 幺 \term{identity}. 如果 $e^{\prime}$ 也是幺, 则
    \begin{align*}
...
\begin{definition}
    ...
    \begin{align*}
        y \circ x = x \circ y = e,
    \end{align*}
    则说 $y$ 是 $x$ 的 (关于运算 $\circ$ 的) 逆 \term{inverse}.
\end{definition}

\begin{example}
    $\mathbb{F}$ 的每个元都有加法逆, 即其相反数.
\end{example}
...
\begin{remark}
    ...
    \end{align*}
    这个规则往往称为袜靴规则 \term{socks and shoes rule}: 设 $y$ 是穿袜, $x$ 是穿靴, $x \circ y$ 表示动作的复合: 先穿袜后穿靴. 那么这个规则告诉我们, $x \circ y$ 的逆就是先脱靴再脱袜.
\end{remark}
...
\begin{example}
    还是看熟悉的 $\mathbb{N}$. 对于乘法而言, 这里的幂就是普通的幂——一个数自乘多次的结果. 对于加法而言, 这里的幂相当于乘法——一个数自加多次的结果.
\end{example}

\begin{definition}
    设 $G$ 关于 $\circ$ 是半群. 若 $G$ 的关于 $\circ$ 的幺存在, 且 $G$ 的任意元都有关于 $\circ$ 的逆, 则 $G$ 是群 \term{group}.
\end{definition}

\begin{example}
    $\mathbb{N}$ 关于加法 (或乘法) 不能作成群. $\mathbb{Z}$ 关于加法作成群, 但关于乘法不能作成群. $\mathbb{F}$ 关于乘法不能作成群, 但 $\mathbb{F}^{\ast}$ 关于乘法作成群. 不过, $\mathbb{F}^{\ast}$ 关于加法不能作成群.
\end{example}
...
\begin{example}
    对于 $\mathbb{F}^{\ast}$ 的乘法而言, 这里的任意整数幂跟普通的整数幂没有任何区别. 我们学习数的负整数幂的时候, 也是借助倒数定义的.
\end{example}
...
\begin{example}
    对加法来说, $\mathbb{Z}$ 是 $\mathbb{F}$ 的子群. 对乘法来说, $\mathbb{Z}^{\ast}$ 不是 $\mathbb{F}^{\ast}$ 的子群.
\end{example}
...
\begin{definition}
    若 $G$ 关于名为 $+$ 的二元运算作成群, 幺 $e$ 读作 ``零元'' 写作 $0$, $x \in G$ 的逆 $x^{-1}$ 读作 ``$x$ 的相反元'' 写作 $-x$, 且 $+$ 适合交换律, 则说 $G$ 是加群 \term{additive group}. 相应地, ``元的幂'' 也应该改为 ``元的倍'': $x^m$ 写为 $mx$. 用加法的语言改写前面的幂的规则, 就得到了倍的规则: 对任意 $x,y \in G$, $m,n \in \mathbb{Z}$, 有
    \begin{align*}
         & (m+n)x = mx + nx, \\
         & m(nx) = (mn)x,    \\
         & m(x+y) = mx + my.
...
\begin{definition}
    设 $f$ 是 $\mathbb{Z}$ 的非空子集 $S$ 到加群 $G$ 的函数. 设 $p$, $q$ 是二个整数. 如果 $p \leq q$, 则记
    \begin{align*}
...
\begin{example}
    设 $f$ 与 $g$ 是 $\mathbb{Z}$ 的非空子集 $S$ 到加群 $G$ 的函数. 因为加群的加法适合结合律与交换律, 所以
    \begin{align*}
...
\begin{remark}
    设 $f(i,j)$ 是 $\mathbb{Z} \times \mathbb{Z}$ 的非空子集到加群 $G$ 的函数. 记
    \begin{align*}
...
\begin{example}
    $\mathbb{Z}$, $\mathbb{F}$ (关于普通加法与乘法) 都是环.
\end{example}

\begin{example}
    全体偶数作成的集也是环. 一般地, 设 $k$ 是整数, 则全体 $nk$ ($n \in \mathbb{Z}$) 作成的集是环.
\end{example}
...
\begin{example}
    在 $\mathbb{Z}$, $\mathbb{F}$ 里, 这些就是我们熟悉的 (部分的) 数的运算律.
\end{example}
...
\begin{example}
    $\mathbb{Z}$, $\mathbb{F}$ 都是整环. 当然, 也有介于 $\mathbb{Z}$ 与 $\mathbb{F}$ 之间的整环. 假如 $s \in \mathbb{C}$ 的平方是整数, 那么全体形如 $x + sy$ ($x,y \in \mathbb{Z}$) 的数作成一个整环.
\end{example}
...
\begin{example}
    再来看一个非整环例. 考虑 $\mathbb{Z} \times \mathbb{Z}$. 设 $a,b,c,d \in \mathbb{Z}$. 规定
    \begin{align*}
         & (a,b) = (c,d) \iff a = b \text{ and } c = d, \\
         & (a,b) + (c,d) = (a+b,c+d),                   \\
         & (a,b)(c,d) = (ac,bd).
    \end{align*}
    可以验证, 在这二种运算下, $\mathbb{Z} \times \mathbb{Z}$ 作成一个交换环, 其加法、乘法幺分别是 $(0,0)$, $(1,1)$. 可是
    \begin{align*}
...
\begin{remark}
    可是, 如果这么定义乘法, 那么 $\mathbb{Z} \times \mathbb{Z}$ 可作为一个整环:
    \begin{align*}
        (a,b)(c,d) = (ac-bd,ad+bc).
    \end{align*}
    事实上, 这就是复数乘法, 因为
...
\begin{example}
    设 $D \subset \mathbb{C}$, 且 $D$ 是整环. 不难看出, $\mathbb{Z} \subset D$.
\end{example}

\subsubsection*{\Products}

\begin{definition}
    设 $f$ 是 $\mathbb{Z}$ 的非空子集 $S$ 到整环 $D$ 的函数. 设 $p$, $q$ 是二个整数. 如果 $p \leq q$, 则记
    \begin{align*}
...
\begin{remark}
    不难看出, $D$ 至少有一个单位 $1$, 因为 $1 \cdot 1 = 1$. 定义里的 $y$ 自然就是 $x$ 的 (乘法) 逆, 其一般记为 $x^{-1}$. $x^{-1}$ 当然也是单位. 二个单位 $x,y$ 的积 $xy$ 也是单位: $(xy)(y^{-1}x^{-1})=1$. 单位的乘法当然适合结合律. 这样, $D$ 的单位作成一个 (乘法) 群. 姑且叫 $D$ 的所有单位作成的集为单位群 \term{unit group} 吧!
\end{remark}
...
\begin{example}
    看全体整数作成的整环 $\mathbb{Z}$. 它恰有二个单位: $1$ 与 $-1$.
\end{example}

\begin{example}
    $\mathbb{F}$ 也是整环. 它有无限多个单位: 任意 $\mathbb{F}^{\ast}$ 的元都是单位.
\end{example}
...
\begin{example}
    不难看出, $\mathbb{F}$ 是域. 这也解释了为什么我们用 $\mathbb{F}$ 表示 $\mathbb{Q}$, $\mathbb{R}$, $\mathbb{C}$ 之一.
\end{example}
...
\begin{example}
    设 $F \subset \mathbb{C}$, 且 $F$ 是域. 不难看出, $\mathbb{Q} \subset F$.
\end{example}
