...
\begin{remark}
    ...
    另一方面, 若命题 $P(n)$ 适合数学归纳法的条件 (ii), 则它当然适合变体的条件 (ii)$^{\prime}$. (读者可思考: 既然 $P(\ell)$ 推出 $P(\ell+1)$, 那么给 $P(\ell)$ ``加条件'' $P(n_0)$, $P(n_0 + 1)$, $\cdots$, $P(\ell-1)$ 是不是也能推出 $P(\ell+1)$? 如果读者仍未理解, 作者举个形象的例: 若 $a=b$, 则 $b=a$. $a=b$ 已经能推出 $b=a$. ``若 $a=b$ 且 $a^2=b^2$, $a^3=b^3$, 则 $b=a$'' 是不是也是对的? 当然. 为什么? 因为我们知道, 就算没有 $a^2=b^2$, $a^3=b^3$, $b=a$ 也是对的——这是由 $a=b$ 推出的, 跟其他的条件无关.) 所以, 若数学归纳法的变体是正确的, 则数学归纳法也是正确的. 换句话说, 数学归纳法与其变体是等价的.
