...
\begin{definition}
    ...
    \end{align*}
    为 $D$ 的多项式函数 \term{polynomial function}. 我们也说, 这个 $f$ 是由 $D$ 上 $x$ 的多项式
    \begin{align*}
        f(x) = a_0 + a_1 x + \cdots + a_n x^n
    \end{align*}
    诱导的多项式函数 \term{the polynomial function induced by $f$}. 不难看出, 若二个多项式相等, 则其诱导的多项式函数也相等.
\end{definition}
...
\begin{remark}
    ...
    \end{align*}
    并称 $f(t)$ 是多项式 $f(x)$ 在点 \term{point} $t$ 的值. 至少, 一方通行 \term{one-way traffic} 是没问题的.
...
\begin{definition}
    设 $f(x)$ 是 $D$ 上 $x$ 的多项式. 若有 $a \in D$ 使 $f(a) = 0$, 则说 $a$ 是 (多项式) $f(x)$ 的根 \term{root}.
\end{definition}
...
\begin{pf}
    $n = 0$ 或 $n = 1$ 时, 我们已经知道这是对的. 用数学归纳法. 假设 $\ell$ 次多项式至多有 $\ell$ 个不同的根. 看 $\ell + 1$ 次多项式 $f(x)$. 如果它没有根, 当然至多有 $\ell + 1$ 个不同的根. 如果它有一个根 $a$, 则存在 $\ell$ 次多项式 $q(x)$ 使
    \begin{align*}
        f(x) = q(x) (x-a).
    \end{align*}
