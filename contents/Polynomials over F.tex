...
再来看整式的带馀除法. 因为 $\FF$ 的每个非零元都是 $\FF$ 的单位, 所以有
...
可以看到, 在 $\FF[x]$ 里, 带馀除法的适用范围更广了. 我们还得到了一个有用的事实.

\begin{proposition}
    整式的带馀除法不因系数的范围变大而改变. 具体地说, 设 $E$, $K$ 是三文字 $\QQ$, $\RR$, $\CC$ 的任意二个, 且 $K \subset E$. 设 $f(x)$, $g(x) \in K[x]$, 且 $f(x) \neq 0$. 设 $K$ 上的整式 $q_1 (x)$, $r_1 (x)$ 适合
    \begin{align*}
