...
在初中, 一般用左 $y$ 右 $x$ 的等式表示函数 (的解析式). 这种表示法强调因变元 \term{dependent variable} $y$ 与自变元 \term{independent variable} $x$ 的关系. 不过, 既然我们有 $f(x)$ 这样的记号, 那么因变元就不必写出了. 并且, 我们在前节提到, 我们不再区分整式与整式函数. 所以, 为方便, 我们用另一种方式叙述这二个问题:
