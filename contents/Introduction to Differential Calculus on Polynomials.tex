...
本节讨论整式的微分学 \term{differential calculus} 初步\myFN{学过 $1$ 元函数微分学的读者可能会觉得本节废话连篇. 不过, 为照顾不熟悉三角不等式及相关知识的读者, 作者也没什么更好的写作思路了.}. 这也是本文的最终节.
...
\begin{proposition}
    ...
    \end{align*}
    用文字描述这句话, 就是: 整式函数在任意有限区间上都是有界的 \term{to be bounded}.
\end{proposition}
...
\begin{pf}
    ...
    根据 ``\RootsOfPolynomials '' 节的结论, 存在整式 $q(x)$ 使
    \begin{align*}
        f(x) = (x - t_0) q(x) + f(t_0).
    \end{align*}
    所以
