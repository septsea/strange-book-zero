...
我们知道, 用一个正整数除\myFN{
    ``$f$ 除 $g$'' 意味着 ``以 $f$ 除 $g$'', 也就是 ``$g$ 除以 $f$''; 以符号表之, 即 $g/f$ 或 $g \div f$. 类似地, ``$b$ 乘 $a$'' 意味着 ``以 $b$ 乘 $a$'', 也就是 ``$a$ 乘以 $b$''; 以符号表之, 即 $a \cdot b$ 或 $a \times b$.

    比如说, 一瓶水的价格为 $5$ 分, 某君与其 $3$ 妻 $4$ 妾各买一瓶水需要支付 $5 + 5 + 5 + 5 + 5 + 5 + 5 + 5 = 5 \times 8 = 40$ 分. 我们读乘法表达式 $5 \times 8$ 作 ``$8$ 乘 $5$'' 或 ``$5$ 乘以 $8$''. 虽然 $8 \times 5$ 也是 $40$, 但是它表达的意思与 $5 \times 8$ 表达的意思不同.
}非负整数, 所得的馀数与商是唯一的. 比方说, $5$ 除 $23$ 的馀数只能是 $3$.
...
在前面的例里, 我们知道, 不存在 $q(x)$, $r(x) \in \mathbb{Z}[x]$, 使
\begin{align*}
    1 + 3x + x^2 = q(x) \cdot 2x + r(x), \quad \deg r(x) < 1.
\end{align*}
