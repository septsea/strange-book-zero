...
\begin{remark}
    读者或许有这样的经验: 不是整数的有理数的加、乘运算似乎没有整数的加、乘容易. 带余除法时, 我们往往会碰到商或余式不是整系数的情形. 如果只是执行一次带余除法, 读者 (也包括作者) 还是可以接受的. 可是, 我们用辗转相除法找二个多项式的最大公因子时, 要执行多次带余除法. 作者愿意解救读者\myFN{这是给不用计算机计算的读者的建议; 如果读者用计算机计算, 这些建议就没什么用了.}.
