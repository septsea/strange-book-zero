...
\begin{pf}
    ...
    我们用算学归纳法证明辅助命题 $P(\ell)$: 任取非负整数 $\ell$, 必有二多项式 $s$, $t$ 使
    \begin{align*}
        r_\ell = sf + tg.
    \end{align*}
    $r_0$ 可写为
...
\begin{pf}
    ...
    (ii) 对 $n$ 用算学归纳法. 显然, $n = 1$ 或 $n = 2$ 时, 命题成立. 设 $n = k$ ($k \geq 2$) 时命题成立, 即: 若 $d_k$ 是 $f_1$, $f_2$, $\cdots$, $f_k$ 的最大公因子, 则存在多项式 $u_1$, $u_2$, $\cdots$, $u_k$ 使
    \begin{align*}
...
\begin{pf}
    对等式左侧的不可约的多项式的数目 $m$ 用算学归纳法. 当 $m = 1$ 时, 有
    \begin{align*}
