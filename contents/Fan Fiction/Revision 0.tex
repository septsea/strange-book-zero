...
为方便, 我们用 $I$ 表示 ``整数集'' 或 ``整式集''\myFN{I 就是 integral domain 的第零个字母.}; 相应地, ``$f$ 是整数'' 或 ``$f$ 是整式'' 统称 ``$f$ 是 $I$ 的元''. 这里的 ``整式'' 是系数是 $\FF$ 的元的整式 (也即 $\FF$ 上的整式)——毕竟, 整系数整式不一定有带余除法, 故没法像 $\FF$ 上的整式那样讨论整系数整式.
