...
\begin{definition}
    设 $r(x)$ 是 ($\FF$ 上 $x$ 的) 有理式\myFN{设 $f(x)$, $g(x)$ 是 $\FF$ 上 $x$ 的整式 (也就是说, $f(x)$, $g(x)$ 的系数都是 $\FF$ 的元), 且 $f(x) \neq 0$. 那么 $\frac{g(x)}{f(x)}$ 就是 $\FF$ 上 $x$ 的有理式 \term{rational expression in $x$ over $\FF$}.}. 设 $A$ 是 $\FF$ 的非空子集, 且适合: 任取 $a \in A$, $r(a)$ 存在. 称函数
    \begin{align*}
         & A \to \FF, \tag*{$r_\mathrm{f} \colon$} \\
         & t \mapsto r(t)
    \end{align*}
    为 $A$ 到 $\FF$ 的有理函数 \term{rational function}. 我们也说, 这个函数是由 ($\FF$ 上 $x$ 的) 有理式 $r(x)$ 诱导的 $A$ 到 $\FF$ 的有理函数 \term{the rational function induced by $r(x)$}.
\end{definition}
