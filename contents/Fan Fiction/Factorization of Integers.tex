...
在前面, 我们稍细致地讨论了不可约的整式\myFN{这里的整式的系数是有理数.}, 并知道, 任取非负整数 $N$, 必有次高于 $N$ 的不可约的整式\myFN{但是, 不可约的复系数整式的次一定是 $1$; 不可约的实系数整式的次一定是 $1$ 或 $2$. 由于作者不假定读者有实分析 (这里的 ``实分析'' 是 ``广义的'': 研究实数的子集到实数的函数的学问) 或复分析 (类似地, ``复分析'' 是研究复数的子集到复数的函数的学问) 的知识, 故作者无法详细地展开这些事实. 读者可参考任意一本讲 ``高等代数'' 的教材. 如果读者不知道什么是 ``分析学'', 那么读者可以粗略地视分析学为 ``微积分''.} (如 $x^{N+1} + 2$). 类似地, 我们也有
\begin{proposition}
